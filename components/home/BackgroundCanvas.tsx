...
const BackgroundCanvas = (props: childProps) => {
  ...
  useEffect(() => {
   ...
   let colors: string[] = ['#FCE6C9', '#FFFFCD', '#FFC0CB']
    ...
    const draw = () => {
      ...
      for (let i = 0; i < arcNum.width + 1; i++) {
        // 上下横向第一个圆弧都是四分之一圆
        if (i === 0) {
          drawR(0, 0, imgSize.width, 1, false)
          drawR(0, canvasSize.height, imgSize.width, 1, true)
          // 上下横向最后一个圆弧都是四分之一圆
        } else if (i === arcNum.width) {
          drawR(canvasSize.width, 0, imgSize.width, 0.5, true)
          drawR(canvasSize.width, canvasSize.height, imgSize.width, 0.5, true)
          // 其余是半圆
        } else {
          drawR(i * imgSize.width * 2, 0, imgSize.width, 1, false)
          drawR(i * imgSize.width * 2, canvasSize.height, imgSize.width, 1, true)
        }
      }
      ...
      for (let i = 0; i < arcNum.height; i++) {
        drawR(0, (imgSize.width + verticalRradius) + i * 2 * verticalRradius, verticalRradius, 2, true)
        drawR(canvasSize.width, (imgSize.width + verticalRradius) + i * 2 * verticalRradius, verticalRradius, 2, false)
      }

      // 连续渲染造就动画效果
      setBgTimeout(setTimeout(() => { draw() }, 200))
    }
